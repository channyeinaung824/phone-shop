...
import { Plus, Search, Pencil, Trash2, ChevronLeft, ChevronRight, Phone, Mail, MapPin } from 'lucide-react';
...
export default function SuppliersPage() {
    ...
    const fetchSuppliers = async (currentPage = page, currentLimit = limit) => {
        try {
            const res = await api.get(`/suppliers?q=${search}&page=${currentPage}&limit=${currentLimit}`);
    ...
    useEffect(() => {
        setPage(1);
        const timer = setTimeout(() => fetchSuppliers(1), 500);
    ...
    return (
        <div className="mt-6 mb-4 flex flex-col gap-6">
                        ...
                        <Table className="w-full min-w-[640px] table-auto text-left text-sm">
                            <TableHeader className="sticky top-0 z-10 bg-white dark:bg-[#202940]">
                                <TableRow className="border-b border-gray-100 dark:border-white/10">
                                    <TableHead className="px-4 py-2 text-xs font-bold uppercase text-gray-400 dark:text-gray-300">Name</TableHead>
                                    <TableHead className="px-4 py-2 text-xs font-bold uppercase text-gray-400 dark:text-gray-300">Phone</TableHead>
                            ...
                            <TableBody>
                                {suppliers.length === 0 ? (
                                    ...
                                    suppliers.map((supplier) => (
                                        <TableRow key={supplier.id} className="border-b border-gray-100 dark:border-white/10 hover:bg-gray-50/50 dark:hover:bg-white/5">
                                            <TableCell className="px-4 py-2 font-semibold text-gray-900 dark:text-gray-100">{supplier.name}</TableCell>
                                            ...
                                            <TableCell className="px-4 py-2 text-right">
                                                <div className="flex justify-end gap-2">
                                                    <Button
                                                        ...
                                                        onClick={() => { setSelectedSupplier(supplier); setDialogOpen(true); }}
                                                    >
            ...
            <SupplierDialog open={dialogOpen} onOpenChange={setDialogOpen} supplier={selectedSupplier} onSuccess={fetchSuppliers} />
            <DeleteConfirmDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen} onConfirm={confirmDelete}
                title="Delete Supplier" description={`Are you sure you want to delete ${supplierToDelete?.name}? This action cannot be undone.`} />
        </div>
    );
}
